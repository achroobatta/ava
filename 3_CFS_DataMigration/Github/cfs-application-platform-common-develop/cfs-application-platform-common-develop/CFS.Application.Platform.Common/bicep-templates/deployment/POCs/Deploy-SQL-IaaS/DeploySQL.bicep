...
module deployVM 'sqlVM.bicep' = [for (virtualmachine, i) in vmArray: if (!empty(virtualmachine.vmName)){
  ...
  params: {
    ...
    imageReferencePublisher: virtualmachine.imageReferencePublisher
    imageReferenceOffer: virtualmachine.imageReferenceOffer
    imageSKU: virtualmachine.imageSKU
    imageReferenceVersion: virtualmachine.imageReferenceVersion
    ...
    workspaceKey: virtualmachine.workspaceKey
    keyVaultURL: virtualmachine.keyVaultURL
    keyVaultName_resourceId: virtualmachine.keyVaultName_resourceId
    KeyEncryptionKeyURL: virtualmachine.KeyEncryptionKeyURL
    KekVaultResourceId: virtualmachine.KekVaultResourceId
  }
  dependsOn: [
    rg
  ]
