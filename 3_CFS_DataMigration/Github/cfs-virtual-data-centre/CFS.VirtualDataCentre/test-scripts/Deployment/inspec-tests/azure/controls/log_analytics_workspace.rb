...
resources = yaml(content: inspec.profile.file("log_analytics_workspaces.yml")).params
control "log_analytics_workspace" do
  impact 0.6
  title "Azure: Confirm standard deployment of LAWS"
  desc "Azure: Confirm standard deployment of LAWS"
  tag "network", "azure"
  if !resources.blank?
    resources.each do |item|
      describe azure_generic_resource(resource_group: item["resource_group"], name: item["name"]) do
        it { should exist }
        its("location") { should cmp item["location"] }
        its("properties.sku.name") { should cmp item["sku"] }
        its("properties.retentionInDays") { should cmp item["retention_in_days"] }
      end
    end
  end
end
