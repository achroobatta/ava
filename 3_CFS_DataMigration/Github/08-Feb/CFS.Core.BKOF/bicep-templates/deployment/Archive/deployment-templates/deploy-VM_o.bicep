...
//This are the parameters that will come from pipeline
...
@batchSize(1)
module deployVM '../../modules/Microsoft.Compute/deployVM_o.bicep'  = [for (vm, i) in vmObject.vmValues: {
  ...
  params: {
    ...
    vmName: 'VM${EnvironmentUpper}${(vm.vmRgLocation == 'australiaeast') ? 'EDC' : 'SDC' }${vm.vmService}${vm.vmInstancePrefix}${vm.vmInstance}'
      ...
      vnetResourceGroup: 'rg-${environmentPrefix}-${(vm.vmRgLocation == 'australiaeast') ? 'edc' : 'sdc' }-${vm.vmRgServiceAbbrv}-00${vm.vnetInstance}'
      ...
      diagstorageName: 'dsto${environmentPrefix}${(vm.vmRgLocation == 'australiaeast') ? 'edc' : 'sdc' }${vm.dstoServiceAbbrv}00${vm.storageAccInstance}'
